...
data = pd.read_csv('avg_orders_per_customer_coordinates.csv')
...
for index, row in data.iterrows():
    ...
    radius = row['Среднее количество заказов на пользователя'] * 10  # Предполагается, что столбец с показателем называется 'Показатель'
