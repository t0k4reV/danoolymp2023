...
df = pd.read_csv('dataset_edlvl.csv')
...
avg_education_by_city = df.groupby('city_nm')['education_level_num'].mean()

cities_parameters['avg_ed_lvl'] = cities_parameters['city_nm'].map(avg_education_by_city)
cities_parameters.to_csv('cities_all_parameters.csv', index=False)

print(avg_education_by_city)
